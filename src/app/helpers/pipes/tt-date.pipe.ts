...
export class TtDatePipe implements PipeTransform {
  transform(date: string): string | null {
    ...
    let passMs = (Date.now() - Date.parse(date) - (3 * timeInMs.hour));

    const someNumber = (arr: number[], num: number) => arr.some(el => el === num);
    ...
    const correctTimeName = (passTime: number, timeNames: TimeNames) => {
      ...
      } else if (someNumber(numbers.forPack2, lastNumber) && !someNumber(numbers.forPack3, preLastNumber)) {
